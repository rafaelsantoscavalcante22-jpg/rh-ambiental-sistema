-- Vista consolidada para a página Financeiro / faturamento: coleta + programação + MTR + aprovação + faturamento operacional.
-- security_invoker: RLS das tabelas base aplica-se ao utilizador da sessão.
...
COMMENT ON VIEW public.vw_faturamento_resumo IS
  'Consolidação para conferência final / faturamento: coleta, cliente, programação, MTR, ticket, pesos, aprovação, faturamento operacional.';
