/**
 * 2000 coletas com fluxo completo até FINALIZADO (programação → financeiro),
 * datas distribuídas de forma uniforme numa janela longa (~24 meses por defeito),
 * para testes com vários períodos e meses diferentes.
 *
 * Por registo: programação + MTR + coleta (campos operacionais até ticket/pesagem) +
 * faturamento emitido + numero_nf + confirmacao_recebimento.
 *
 * Uso:
 *   npx tsx scripts/seed-2000-coletas-finalizadas-multimes.ts
 *
 * Ou: npm run seed:2000-coletas-finalizadas-multimes
 *
 * Requer: pelo menos um cliente; VITE_SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY no .env
 *
 * Opcional antes (limpa operação): npx tsx scripts/reset-coletas-operacao.ts --yes
 */
...
const TOTAL = 2000
/** ~24 meses de calendário (distribuição linear das 2000 coletas). */
const DIAS_JANELA = 730
const PREFIX = '[HIST-2000-MULTI]'
...
function camposOperacionais(
  ...
  if (i >= indiceEtapaFluxo('LOGISTICA_DESIGNADA')) {
    out.motorista_nome = motoristaNome
    out.motorista = motoristaNome
    out.placa = placa
  }

  if (i >= indiceEtapaFluxo('TARA_REGISTRADA')) {
    out.peso_tara = 12000 + (seqIndex % 8) * 50
  }

  if (i >= indiceEtapaFluxo('BRUTO_REGISTRADO')) {
    out.peso_bruto = 18500 + (seqIndex % 12) * 30
    out.peso_liquido = 6500 + (seqIndex % 40) * 25
  }

  if (i >= indiceEtapaFluxo('TICKET_GERADO')) {
    out.ticket_numero = `TK-H2K-${String(seqIndex).padStart(4, '0')}`
  }
...
function camposObrigatoriosBanco(): Record<string, unknown> {
  return {
    ...
    endereco: `Rua Historico 2000 multimes, ref. ${ETAPA}`,
...
/**
 * Distribui os índices 0..total-1 linearmente ao longo de DIAS_JANELA dias
 * (do mais antigo ao mais recente), cobrindo muitos meses diferentes.
 */
function dataAgendadaParaIndice(k: number): string {
  ...
  const diasOffset = Math.floor((k / (TOTAL - 1)) * (DIAS_JANELA - 1))
  const d = new Date(inicio)
  d.setDate(d.getDate() + diasOffset)
  return formatarDataYmd(d)
}

/** Hora do created_at variada para não ficar tudo idêntico. */
function createdAtIso(dataAgendada: string, k: number): string {
  const h = 8 + (k % 10)
  const min = (k * 13) % 60
  return `${dataAgendada}T${String(h).padStart(2, '0')}:${String(min).padStart(2, '0')}:00.000Z`
}
...
/** Placa estilo Mercosul (LLLNLNN) quando não há caminhões cadastrados. */
function placaFallback(seqIndex: number): string {
  const a = String.fromCharCode(65 + (seqIndex % 26))
  const b = String.fromCharCode(65 + ((seqIndex >> 2) % 26))
  const c = String.fromCharCode(65 + ((seqIndex >> 4) % 26))
  const d = 1 + (seqIndex % 9)
  const e = String.fromCharCode(65 + ((seqIndex >> 1) % 26))
  const f = seqIndex % 10
  const g = (seqIndex * 3) % 10
...
async function main() {
  ...
  const { data: motoristasRows } = await supabase.from('motoristas').select('nome').order('nome')
  const nomesMotoristas = (motoristasRows || [])
    .map((r) => (r as { nome?: string }).nome)
    .filter((n): n is string => Boolean(n?.trim()))

  const { data: caminhoesRows } = await supabase.from('caminhoes').select('placa').order('placa')
  const placas = (caminhoesRows || [])
    .map((r) => (r as { placa?: string }).placa)
  ...
  console.log(
    `Janela: ${DIAS_JANELA} dias (~${Math.round(DIAS_JANELA / 30)} meses), ${TOTAL} coletas, etapa ${ETAPA}.`
  )
  if (nomesMotoristas.length) console.log(`Motoristas da base: ${nomesMotoristas.length} (rotação).`)
  if (placas.length) console.log(`Placas da base: ${placas.length} (rotação).`)
  ...
  for (let k = 0; k < TOTAL; k++) {
    ...
    const numeroProg = `${PREFIX}-${runStamp}-${String(k + 1).padStart(5, '0')}`
    const numeroColeta = baseNumero + k
    const valorColeta = 2500 + (k % 120) * 75
    const refNf = `NF-H2K-${numeroColeta}`

    const motoristaNome =
      nomesMotoristas.length > 0 ? nomesMotoristas[k % nomesMotoristas.length]! : 'Motorista Historico 2000'
    const placa = placas.length > 0 ? placas[k % placas.length]! : placaFallback(k)
      ...
      .insert([
        {
          ...
          tipo_caminhao: 'Truck',
          tipo_servico: 'Coleta',
          observacoes: `${PREFIX} janela ${DIAS_JANELA}d — caso ${k + 1}/${TOTAL}`,
          coleta_fixa: false,
          periodicidade: null,
    ...
    const { data: mtrRow, error: eMtr } = await supabase
      ...
      .insert([
        {
          numero: `MTR-H2K-${numeroColeta}`,
          programacao_id: programacaoId,
          cliente: cliente.nome,
          gerador: cliente.nome,
          endereco: 'Rua Historico 2000 multimes',
    ...
    const createdAt = createdAtIso(dataAgendada, k)

    const row: Record<string, unknown> = {
      ...montarPayloadBanco(),
      ...camposOperacionais(dataAgendada, k, motoristaNome, placa),
  ...
  console.log(
    `Periodo: ${DIAS_JANELA} dias com distribuicao linear — ${ETAPA} — NF + confirmacao recebimento — pagamento Pago — faturamento emitido.\n`
  )
}
